...
const objLat = (obj) => {
    const lastNameCapitalized = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1);
    const firstNameCapitalized = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1);
    return 'my name is '+ firstNameCapitalized +" "+ lastNameCapitalized + ' I am ' + obj.age+ ' YO, and I love ' + obj.hobby +".";
};
...
for (let i = 0 ; i<arr.length ; i++){
    if (arr[i].yearsOfExperience  > 1){
        if(arr[i].firstName==null){
            newaArray.push({"fullName":arr[i].lastName,"tech":arr[i].tech});
        }else if(arr[i].lastName==null){
            newaArray.push({"fullName":arr[i].firstName,"tech":arr[i].tech});

        }else{
            newaArray.push({"fullName":arr[i].firstName +" " +arr[i].lastName,"tech":arr[i].tech});
...
const applicationsStatics = (arr) => {
    ...
    var rejectedApplicants=0, python_Devs=0, javaScript_Devs=0, dotNet_Devs=0, java_Devs=0, totalApplicants =0;
    for(let i =0 ; i<arr.length;i++){
        totalApplicants++;
        if(arr[i].tech =="JS")
        {
